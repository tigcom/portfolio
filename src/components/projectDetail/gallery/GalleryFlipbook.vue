<template>
  <div class="pg-flipbook">
    <BlossomCarousel ref="carouselRef" as="ul" class="pg-flipbook__viewport">
      <li
        v-for="(img, i) in images"
        :key="i"
        data-blossom-slide
        class="pg-flipbook__slide"
        :class="{ 'is-active': i === activeIndex }"
      >
        <button
          class="pg-flipbook__card"
          :aria-label="`Zoom ${title} — screen ${i + 1}`"
          @click="$emit('open', i)"
        >
          <img :src="img" :alt="`${title} — screen ${i + 1}`" loading="lazy" />
          <span class="pg-flipbook__num">{{ pad(i + 1) }}</span>
        </button>
      </li>
    </BlossomCarousel>
  </div>
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════ */
.pg-flipbook__viewport {
  --fb-w: min(74vw, 1000px);
